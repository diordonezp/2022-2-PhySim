...
public:
  ...
  double GetEc(void){return m*V.norm2()/2+I*omega*omega/2;};
  double GetV(void){return V.norm();};
...
void Colisionador::CalculeFuerzas(Cuerpo *Grano,double dt){
  int i,j;
  for(i=N+1;i<N+3;i++)
      {Grano[i].F.load(0,0,0); Grano[i].tau=0;}
  Grano[N+3].F.load(0,-Grano[N+3].m*g,0); Grano[i].tau=0;
  for(i=0;i<N;i++)  
    {Grano[i].F.load(0,-Grano[i].m*g,0); Grano[i].tau=0;}
  for(i=0;i<N;i++)  
    for(j=i+1;j<N+4;j++)
...
void Colisionador::Choque(Cuerpo &Cuerpo1,Cuerpo &Cuerpo2,double &dcontacto,double &hold,double dt){
  ...
  if(h>0){//si hay contacto
    ...
    n=r12/r12.norm();  Rw.load(0,0,R1*Cuerpo1.omega+R2*Cuerpo2.omega);
    Vc=V12-(Rw^n); Vn=Vc*n; t.load(n.y(),-n.x(),0); Vt=Vc*t;
