...
//---- declarar constantes ---
const double K=1.0e8;
const double Lx=12, Ly=12;
const int Nx=1, Ny=1, N=Nx*Ny;

const double g=9.8, Gamma=1000, Kcundall=900, mu=0.4;
...
int main(void){
  ...
  int i,ix,iy;
  double t,tdibujo,dt=1e-6,tmax=50,tcuadro=tmax/100000;
  ...
  double Rpared=1000*Lx, Mpared=100*m0;
  ...
  for(ix=0;ix<Nx;ix++)
    for(iy=0;iy<Ny;iy++){
      ...
      //-----------------------(   x0,   y0,          Vx0,          Vy0, theta0,omega0  ,m0,R0)
      Grano[Nx*iy+ix].Inicie((ix+1)*dx,R0,    1,            0,      0,OmegaMax,m0,R0);//OJO
    }
