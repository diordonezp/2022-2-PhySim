...
//---- declarar constantes ---
const double g=9.8, K=1.0e4, Gamma=10, Kcundall=500, MU=0.4;
...
class Colisionador{
private:
  double xcundall[N+4][N+4], hold[N+4][N+4];
public:
  void inicie();
...
int main(void){
  ...
  for(ix=0;ix<Nx;ix++)
    for(iy=0;iy<Ny;iy++){
      ...
      //--------------------(       x0,       y0,               Vx0,               Vy0, m0,R0,theta0,omega0)
      Grano[Nx*iy+ix].Inicie((ix+1)*dx,(iy+1)*dy,0/*V0*cos(Theta)*/,0/*V0*sin(Theta)*/, m0,R0,0,omegamax);//OJO
    }
